...
\label{ch:06}
Program mikrokontrolera \textit{ESP32} został napisany w języku \textit{C++} za pomocą aplikacji
\textit{Visual Studio Code} wykorzystującej, dedykowane programowaniu mikrokontrolerów,
rozszerzenie \textit{PlatformIO}. Kod wynikowy zajmuje około 900~kB. 
...
\begin{figure}[H]
  ...
  \caption{Ogólny diagram sekwencji działania oprogramowania. Źródło: opracowanie własne.}
...
\label{sec:obsluga-led}
  ...
  \begin{figure}[H]
    ...
    \caption{Diagram sekwencji działania menadżera diod LED. Źródło: opracowanie własne.}
...
\label{sec:segment-display}
  ...
  \begin{figure}[H]
    ...
    \caption{Wizualizacja schematu działania wyświetlaczy segmentowych. Źródło: opracowanie własne.}
...
\label{sec:filesystem}
  ...
  pamięci \textit{flash} urządzenia dla systemu plików \cite{bib:littlefs}. Zapisane w ten sposób dane nie ulegają usunięciu po odłączeniu zasilania urządzenia.
...
\label{sec:w-server}
  ...
  \begin{itemize}
    ...
    \item tworzenie serwera \textit{DNS},
  ...
  \begin{figure}[H]
    ...
    \caption{Uproszczony diagram sekwencji działania trybu sieciowego. Źródło: opracowanie własne.}
    \label{fig:schemat-web-mode}
  \end{figure}

  Sposób działania pomija szczegóły zwiazane z inicjalizacją poszczególnych komponentów takich jak: tworzenie serwera \textit{DNS},
...
\label{sec:w-local}
  ...
  \begin{figure}[H]
    ...
    \caption{Uproszczony diagram sekwencji działania trybu lokalnego. Źródło: opracowanie własne.}
  ...
  \textit{czyt/pisz}, \textit{wyak/wys} lub \textit{wyl/wyad}. W przypadku wykrycia wciśnięcia niedozwolonego przycisku urządzenie nie doda rozkazu do listy.
