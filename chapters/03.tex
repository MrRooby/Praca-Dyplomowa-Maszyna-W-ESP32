...
\label{sec:esp32}
  ...
  \clearpage
  Po rozważeniu obu opcji zdecydowano się na użycie mikrokontrolera z rodziny \textit{ESP32}. Wybór został podyktowany pozwalającym na pełną 
  ...
  \includegraphics[width=0.7\textwidth]{./graf/schemat-esp32.png}
    \caption{Schemat blokowy funkcjonalny układu \textit{ESP32-S3}. 
...
\label{sec:serwer}
  ...
  \begin{itemize}
    ...
    \item \textit{AsyncTCP} - asynchroniczna, nieblokująca komunikacja \textit{TCP} 
      (z ang. \textit{Transmission Control Protocol}),
    \item \textit{AsyncWebServer} - asynchroniczna obsługa serwera \textit{HTTP} 
      (z ang. \textit{Hypertext Transfer Protocol}),
    \item \textit{LittleFS} - system plików dla pamięci \textit{flash} mikrokontrolera,
    \item \textit{DNSServer} - lokalna obsługa i przekierowywanie zapytań \textit{DNS} 
  ...
  \begin{itemize}
    ...
    \item komunikację przy wykorzystaniu protokołu sieciowego \textit{TCP},
    \item przechowywanie złożonej strony internetowej Maszyny W bezpośrednio w pamięci \textit{flash} mikrokontrolera,
    \item wykorzystanie portalu przechwytującego zapytania \textit{DNS} w celu przekierowania użytkowników na wybraną stronę.
  \end{itemize} 
...
\section{Zasilanie}
  ...
  odpowiedniego zasilacza. Adresowalne diody LED oraz podświetlenie wymagały zasilania napięciem 12~V, natomiast mikrokontroler \textit{ESP32} 
  ...
  \begin{itemize}
    ...
    \item dla mikrokontrolera \textit{ESP32-S3} oraz układów towarzyszących przewidziano pobór prądu około 600~mA przy pełnym obciążeniu interfejsów peryferyjnych i aktywnym WiFi.
  \end{itemize}
...
Zasilanie mikrokontrolera \textit{ESP32} oraz innych układów niskonapięciowych realizowano przy użyciu przetwornicy DC-DC obniżającej napięcie z 12~V do 3,3~V.
