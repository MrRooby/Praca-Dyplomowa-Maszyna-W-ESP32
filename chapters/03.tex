...
\label{sec:esp32}
  ...
  Z katalogu firmy \textit{Espressif Systems} wybrano urządzenie ESP32-S3-WROOM-1.
...
\label{sec:led-rgb}
  ...
  Spośród aktualnie dostępnych na rynku wersji elementu wyróżniły się dwa modele: WS2812B oraz WS2815B firmy
  \textit{WORLDSEMI}.
  Podczas wstępnych testów obu elementów zaobserwowano następujące cechy. Diody WS2812B zasilane napięciem 5~V wykazywały
  zauważalny spadek jakości odwzorowania kolorów przy liczbie elementów przekraczającej 100 (w ciągłej linii). Wykorzystanie 
  ww. elementów niosłoby za sobą potrzebę dodatkowego wprowadzania napięcia do linii diod. Diody WS2815B zasilane napięciem 
  ...
  modelu WS2815B-V1, jego jedyną wadą jest nieznacznie większa cena w porównaniu do 5~V odpowiednika. 
  \\
...
\label{sec:przyciski}
  ...
  wybrano model CD74HC4067M firmy \textit{Texas Instruments}.
  ...
  \begin{figure}[H]
    ...
    \caption{Diagram funkcjonalny multipleksera CD74HC4067M. 
    % Źródło: \url{https://www.alldatasheet.com/html-pdf/517535/TI/CD74HC4067M/62/2/CD74HC4067M.html}}
      Źródło: \cite{bib:mux-zdjecie}}
    \label{fig:schemat-mux}
  \end{figure}
  
  Poniższa tablica (Tab. \ref{tab:mux-truth-table}) prawdy przedstawia odczyt stanu przycisków.
  \begin{table}[H]
    \centering
...
\begin{figure}[H]
  ...
  \begin{minipage}[t]{0.58\textwidth} % Zwiększona szerokość dla rysunku
    ...
    \caption{Wykres stanów przy obrocie w prawo. Źródło: opracowanie własne na podstawie \cite{bib:enc-zdjecie}.}
    \label{fig:enc-cw}
  \end{minipage}
  \hfill
  ...
  \begin{minipage}[t]{0.58\textwidth}
    ...
    \caption{Wykres stanów przy obrocie w lewo. Źródło: opracowanie własne na podstawie \cite{bib:enc-zdjecie}.}
    \label{fig:enc-ccw}
  \end{minipage}
  \hfill
...
\section{Punkt dostępowy}
  ...
  Biblioteki wymienione w rozdziale \ref{sec:serwer} umożliwiają zadanie hasła punktowi dostępu oraz ograniczenie liczby połączonych na raz użytkowników.
...
\section{Strona internetowa}
  ...
  \includegraphics[width=1\textwidth]{./graf/maszyna-web.png}
    \caption{Strona główna sieciowego symulatora Maszyny W. Źródło: \cite{bib:aei-maszynaw}}
  ...
  przy pomocy wymienionych w rozdziale \ref{sec:led-rgb} wyświetlaczy. Kontrolę sygnałów takich jak: czyt, wys, wei, itd. umożliwią 
...
\section{Interfejs komunikacyjny}
  ...
  \begin{figure}[H]
    ...
    \caption{Schemat komunikacji przy wykorzystaniu protokołu \textit{WebSocket}. Źródło: opracowanie własne na podstawie \cite{bib:websocket-zdjecie}.}
